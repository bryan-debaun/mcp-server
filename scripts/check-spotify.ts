...
async function run() {
    ...
    if (!clientId || !clientSecret || !refreshToken) {
        console.error(
            'Missing SPOTIFY_CLIENT_ID / SPOTIFY_CLIENT_SECRET / SPOTIFY_REFRESH_TOKEN in environment',
        )
        process.exitCode = 2
        return
    }
    ...
    try {
        const tokenRes = await exchangeRefreshToken(
            clientId,
            clientSecret,
            refreshToken,
        )
        ...
        try {
            ...
            try {
                ...
                console.log(
                    'getLikedTracks result (trimmed):',
                    JSON.stringify(l, null, 2).slice(0, 800),
                )
            } catch (err: any) {
            ...
            console.error(
                'spotify-adapter module not importable from src — is this a compiled/runtime mismatch?',
                (err as any)?.message ?? err,
            )
        }
    } catch (err) {
