...
async function main() {
    ...
    const res2 = await c.query(
        `SELECT policyname, tablename, cmd, qual IS NOT NULL as has_using, with_check IS NOT NULL as has_with_check FROM pg_policies WHERE schemaname='public' AND tablename = ANY($1) ORDER BY tablename, policyname`,
        [tables],
    )
    ...
    for (const row of mig.rows) {
        const shortenedLogs = row.logs
            ? row.logs.length > 200
                ? row.logs.slice(0, 200) + '...'
                : row.logs
            : null
        console.log({
