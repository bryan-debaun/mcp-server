...
async function main() {
    ...
    if (!email) {
        console.error(
            'Usage: pnpm exec tsx scripts/create-supabase-user-for-local.ts <email>',
        )
        process.exit(2)
    }
    ...
    if (user.external_id) {
        console.error(
            'Local user already linked to Supabase:',
            user.external_id,
        )
        process.exit(0)
    }
    ...
    if (!supabaseUrl || !supabaseKey) {
        console.error(
            'PUBLIC_SUPABASE_URL (or SUPABASE_ISS) and SUPABASE_SECRET_KEY (or SUPABASE_SERVICE_ROLE_KEY) are required',
        )
        process.exit(1)
    }
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (!supId) {
                console.error(
                    'Supabase did not return an id for new user',
                    payload,
                )
                process.exit(1)
            }
        ...
        const lookup = await fetch(
            `${supabaseUrl.replace(/\/$/, '')}/auth/v1/admin/users?email=${encodeURIComponent(user.email)}`,
            {
                headers: {
                    Authorization: `Bearer ${supabaseKey}`,
                    apikey: supabaseKey,
        ...
        if (lookup.ok) {
            ...
            if (found && found.id) {
                await prisma.profile.update({
                    where: { id: user.id },
                    data: { external_id: found.id },
                })
                console.log(
                    'Found existing Supabase user and linked local user ->',
                    found.id,
                )
                process.exit(0)
            }
