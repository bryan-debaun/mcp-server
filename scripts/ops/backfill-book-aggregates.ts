...
export default async function backfill(batchSize = Number(argv['batch-size'])) {
    await initPrisma()
    console.log(
        `Starting backfill of book aggregates (batchSize=${batchSize}) dryRun=${argv['dry-run']}`,
    )
    ...
    while (true) {
        ...
        for (const b of books) {
            lastId = b.id
            const agg: any =
                await prisma.$queryRaw`SELECT COUNT(*)::int AS count, ROUND(AVG("rating")::numeric, 2) AS avg FROM "Rating" WHERE "bookId" = ${b.id}`
            const count = Number(agg?.[0]?.count ?? 0)
            const avg =
                agg?.[0]?.avg !== null && agg?.[0]?.avg !== undefined
                    ? Number(agg[0].avg)
                    : null
            ...
            if (!argv['dry-run']) {
                ...
                console.log(
                    `[dry-run] book ${b.id} -> count=${count} avg=${avg}`,
                )
            }
    ...
    try {
        ...
        if (!argv['dry-run']) {
            metrics.bookAggregatesLastBackfillTimestamp?.set?.(
...
if (
    process.argv[1] &&
    process.argv[1].endsWith('backfill-book-aggregates.ts')
) {
    backfill().catch((err) => {
