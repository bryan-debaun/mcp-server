...
if (isProd && !argv.force) {
    console.error(
        'Refusing to run smoke check against production without --force. Pass --force and --confirm-token=REALLY_I_AGREE to proceed.',
    )
    process.exit(1)
}
...
export default async function smoke() {
    ...
    const aggCountRaw: any =
        await prisma.$queryRaw`SELECT COUNT(*)::int as c FROM "RatingAggregate"`
    ...
    const sampleAggs: any =
        await prisma.$queryRaw`SELECT entity_type, entity_id, rating_count, average_rating FROM "RatingAggregate" LIMIT 3`
