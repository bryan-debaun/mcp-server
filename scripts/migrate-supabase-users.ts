...
type Opts = {
    dryRun: boolean
    mode: 'create-missing' | 'recreate' | 'wipe'
    confirm?: string
    force?: boolean
}
...
async function fetchSupabaseUsers(supabaseUrl: string, supabaseKey: string) {
    ...
    while (true) {
        ...
        if (!res.ok)
            throw new Error(`failed to list supabase users: ${res.status}`)
...
async function createMissingLocalUsersLinkedToSupabase(
    supabaseUrl: string,
    supabaseKey: string,
    dryRun: boolean,
) {
    const localUsers = await prisma.profile.findMany({
        where: { external_id: null, blocked: false },
    })
    console.error(
        `Found ${localUsers.length} local user(s) without external_id`,
    )
    for (const u of localUsers) {
        ...
        if (existingSup && existingSup.id) {
            console.error(
                `  -> Found existing Supabase user id=${existingSup.id}; will write external_id`,
            )
            if (!dryRun)
                await prisma.profile.update({
                    where: { id: u.id },
                    data: { external_id: existingSup.id },
                })
            continue
        }
        ...
        if (!res.ok) {
            ...
            console.error(
                `  !! Supabase create failed for ${u.email}: ${res.status} ${txt}`,
            )
            continue
        }
...
async function recreateLocalFromSupabase(
    supabaseUrl: string,
    supabaseKey: string,
    dryRun: boolean,
    force: boolean,
    ...
    if (!dryRun && confirm !== 'REALLY_I_AGREE') {
        console.error(
            'Destructive operation. Pass --confirm REALLY_I_AGREE to proceed.',
        )
        process.exit(2)
    }
    ...
    if (dryRun) {
        const nonAdminCount = await prisma.profile.count({
            where: { isAdmin: false },
        })
        console.error(
            `Dry-run: would remove ${nonAdminCount} local non-admin user(s) and recreate ${supUsers.length} users from Supabase.`,
        )
        console.error(
            'Example supabase users:',
            supUsers.slice(0, 10).map((u: any) => u.email),
        )
        return
    }
    ...
    for (const s of supUsers) {
        try {
            ...
            const created = await prisma.profile.create({
                data: { email, name, roleId: role.id, external_id: s.id },
            })
            console.error(
                `  created local user ${email} (id=${created.id} external_id=${s.id})`,
            )
        } catch (err: any) {
            console.error(
                '  failed to create local user for',
                s.email,
                err?.message ?? err,
            )
        }
...
async function wipeLocalUsers(
    ...
    if (dryRun) {
        const count = await prisma.profile.count({
            where: force ? {} : { isAdmin: false },
        })
        console.error(
            `Dry-run: would delete ${count} local user(s) (force=${Boolean(force)})`,
        )
        return
    }
...
async function main() {
    const supabaseUrl =
        process.env.PUBLIC_SUPABASE_URL ?? process.env.SUPABASE_ISS
    const supabaseKey =
        process.env.SUPABASE_SECRET_KEY ?? process.env.SUPABASE_SERVICE_ROLE_KEY
    console.error('Supabase URL from env:', supabaseUrl)
    if (!supabaseUrl || !supabaseKey) {
        console.error(
            'PUBLIC_SUPABASE_URL (or SUPABASE_ISS) and SUPABASE_SECRET_KEY (or SUPABASE_SERVICE_ROLE_KEY) are required to run this script',
        )
        process.exit(1)
    }
    ...
    try {
        ...
        console.error(
            'initPrisma failed (continuing with stubbed prisma):',
            err,
        )
    }
    ...
    if (opts.mode === 'create-missing') {
        ...
        await createMissingLocalUsersLinkedToSupabase(
            supabaseUrl,
            supabaseKey,
            opts.dryRun,
        )
    ...
    if (opts.mode === 'recreate') {
        ...
        await recreateLocalFromSupabase(
            supabaseUrl,
            supabaseKey,
            opts.dryRun,
            opts.force ?? false,
