...
CREATE POLICY "Movie_creator_or_admin_insert" ON "Movie" FOR INSERT WITH CHECK (
  ...
  OR (current_setting('request.jwt.claims.email', true) IS NOT NULL
      AND (
        exists (select 1 from "Profile" p where p.email = current_setting('request.jwt.claims.email', true) and p.id = "createdBy")
        OR exists (select 1 from "User" u where u.email = current_setting('request.jwt.claims.email', true) and u.id = "createdBy")
      ))
);
...
CREATE POLICY "Movie_creator_or_admin_update" ON "Movie" FOR UPDATE USING (
  ...
  OR (current_setting('request.jwt.claims.email', true) IS NOT NULL
      AND (
        exists (select 1 from "Profile" p where p.email = current_setting('request.jwt.claims.email', true) and p.id = "Movie"."createdBy")
        OR exists (select 1 from "User" u where u.email = current_setting('request.jwt.claims.email', true) and u.id = "Movie"."createdBy")
      ))
) WITH CHECK (
  current_setting('request.jwt.claims.role', true) = 'admin'
  OR (current_setting('request.jwt.claims.email', true) IS NOT NULL
...
CREATE POLICY "VideoGame_creator_or_admin_update" ON "VideoGame" FOR UPDATE USING (
  ...
  OR (current_setting('request.jwt.claims.email', true) IS NOT NULL
      AND (
        exists (select 1 from "Profile" p where p.email = current_setting('request.jwt.claims.email', true) and p.id = "VideoGame"."createdBy")
        OR exists (select 1 from "User" u where u.email = current_setting('request.jwt.claims.email', true) and u.id = "VideoGame"."createdBy")
      ))
) WITH CHECK (
  current_setting('request.jwt.claims.role', true) = 'admin'
  OR (current_setting('request.jwt.claims.email', true) IS NOT NULL
...
CREATE POLICY "ContentCreator_creator_or_admin_update" ON "ContentCreator" FOR UPDATE USING (
  ...
  OR (current_setting('request.jwt.claims.email', true) IS NOT NULL
      AND (
        exists (select 1 from "Profile" p where p.email = current_setting('request.jwt.claims.email', true) and p.id = "ContentCreator"."createdBy")
        OR exists (select 1 from "User" u where u.email = current_setting('request.jwt.claims.email', true) and u.id = "ContentCreator"."createdBy")
      ))
) WITH CHECK (
  current_setting('request.jwt.claims.role', true) = 'admin'
  OR (current_setting('request.jwt.claims.email', true) IS NOT NULL
