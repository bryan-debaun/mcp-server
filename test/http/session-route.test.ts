    ...
    describe('GET /api/auth/session', () => {
        ...
        beforeAll(async () => {
            ...
            try {
                ...
                // Stub profile.findUnique (supports lookups by id or email)
                prisma.profile.findUnique = async ({ where, _include }: any) => {
                    if (where.id !== undefined) return usersById[String(where.id)] ?? null
        ...
        it('returns user info for signed token', async () => {
            ...
            expect(res.body).toMatchObject({ id: user.id, email: user.email, isAdmin: false })
        })
