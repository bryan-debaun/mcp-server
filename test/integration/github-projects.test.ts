...
const TEST_PROJECT_NUMBER = parseInt(
    process.env.GITHUB_TEST_PROJECT_NUMBER || '2',
    10,
)
const TEST_ISSUE_NUMBER = parseInt(
    process.env.GITHUB_TEST_ISSUE_NUMBER || '73',
    10,
)
...
describe('GitHub Projects V2 Integration', () => {
    ...
    beforeAll(() => {
        ...
        if (!process.env.GITHUB_TOKEN) {
            throw new Error(
                'GITHUB_TOKEN environment variable required for integration tests',
            )
        }
        ...
        console.log(
            `  Test Issue: ${TEST_OWNER}/${TEST_REPO}#${TEST_ISSUE_NUMBER}`,
        )
    })
    ...
    describe('create-project-field', () => {
        it('should create a TEXT field', async () => {
            if (!testProjectId) {
                throw new Error(
                    'testProjectId not set - run get-project-fields test first',
                )
            }
            ...
            console.log(
                `Created TEXT field: ${testFieldName} (${result.fieldId})`,
            )
        }, 30000)
        ...
        it('should create a SINGLE_SELECT field with options', async () => {
            ...
            const createdField = fields.fields.find(
                (f) => f.id === result.fieldId,
            )
            ...
            console.log(
                `Created and cleaned up SINGLE_SELECT field: ${testFieldName}`,
            )
        }, 30000)
    })
    ...
    describe('update-project-field', () => {
        it('should rename a field', async () => {
            ...
            try {
                // Get current name
                clearProjectCache(TEST_OWNER, TEST_PROJECT_NUMBER)
                const fieldsBefore = await getProjectFields(
                    TEST_OWNER,
                    TEST_PROJECT_NUMBER,
                )
                const fieldBefore = fieldsBefore.fields.find(
                    (f) => f.id === fieldToRename,
                )
                expect(fieldBefore).toBeDefined()

                const newName = `${fieldBefore!.name}_Renamed`
                await updateProjectFieldName(
                    testProjectId,
                    fieldToRename,
                    newName,
                )
                ...
                clearProjectCache(TEST_OWNER, TEST_PROJECT_NUMBER)
                const fieldsAfter = await getProjectFields(
                    TEST_OWNER,
                    TEST_PROJECT_NUMBER,
                )
                const fieldAfter = fieldsAfter.fields.find(
                    (f) => f.id === fieldToRename,
                )
    ...
    describe('add/remove field options', () => {
        ...
        beforeAll(async () => {
            ...
            console.log(
                `Created SINGLE_SELECT field for option testing: ${selectFieldId}`,
            )
        })
        ...
        afterAll(async () => {
            ...
            if (testProjectId && selectFieldId) {
                ...
                console.log(
                    `Cleaned up SINGLE_SELECT test field: ${selectFieldId}`,
                )
            }
        })
        ...
        it.skip('should add options to SINGLE_SELECT field', async () => {
            ...
            await addFieldOptions(testProjectId, selectFieldId, [
                'NewOption1',
                'NewOption2',
            ])
            ...
            console.log(
                `Added 2 options. Total options: ${field!.options?.length}`,
            )
        }, 30000)
        ...
        it.skip('should remove options from SINGLE_SELECT field', async () => {
            ...
            expect(fieldAfter!.options?.length).toBe(
                fieldBefore!.options!.length - 1,
            )

            console.log(
                `Removed option. Remaining options: ${fieldAfter!.options?.length}`,
            )
        }, 30000)
    })
    ...
    describe('set-project-field-value', () => {
        it('should add issue to project and set field value', async () => {
            ...
            console.log(
                `Set field value on issue #${TEST_ISSUE_NUMBER}: ${testValue}`,
            )
        }, 30000)
    })
    ...
    describe('field caching', () => {
        it('should cache project fields on repeated calls', async () => {
            ...
            const result1 = await getProjectFields(
                TEST_OWNER,
                TEST_PROJECT_NUMBER,
            )
            ...
            const result2 = await getProjectFields(
                TEST_OWNER,
                TEST_PROJECT_NUMBER,
            )
            ...
            console.log(
                `First call: ${duration1}ms, Cached call: ${duration2}ms`,
            )
