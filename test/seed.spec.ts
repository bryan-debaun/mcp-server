...
describe('prisma seed', () => {
    ...
    it('marks existing ADMIN_EMAIL user as admin', async () => {
        ...
        mockPrisma.profile.findUnique = vi
            .fn()
            .mockResolvedValue({ id: 2, email: 'foo@example.com' })
        ...
        expect(consoleLogSpy).toHaveBeenCalledWith(
            expect.stringContaining('Marked existing user'),
        )
    })
    ...
    it('creates minimal user for ADMIN_EMAIL when not present', async () => {
        ...
        expect(mockPrisma.profile.create).toHaveBeenCalledWith({
            data: { email: 'new@example.com', isAdmin: true },
        })
        expect(consoleWarnSpy).toHaveBeenCalledWith(
            expect.stringContaining(
                'Created minimal users row for ADMIN_EMAIL',
            ),
        )
    })
    ...
    it('performs seed when admin role missing', async () => {
        ...
        expect(mockPrisma.book.upsert).toHaveBeenCalledWith(
            expect.objectContaining({
                create: expect.objectContaining({ status: 'NOT_STARTED' }),
            }),
        )
    })
    ...
    it('handles existing movie with same iasn but different id without throwing', async () => {
        ...
        mockPrisma.role.findUnique.mockResolvedValue(null)
        mockPrisma.movie.upsert = vi
            .fn()
            .mockResolvedValue({ id: 99, iasn: 'IASN-001' })

        await expect(runSeed(mockPrisma)).resolves.not.toThrow()
        expect(mockPrisma.movie.upsert).toHaveBeenCalledWith(
            expect.objectContaining({ where: { iasn: 'IASN-001' } }),
        )
    })
