...
vi.mock('../../src/config.js', () => ({ config: { database: { url: undefined } } }))
...
describe('Prisma stub coverage', () => {
    it('every schema.prisma model has a DB-less stub (guards against drift)', async () => {
        ...
        for (const model of models) {
            ...
            expect(prisma[key], `missing DB-less stub for schema model "${model}" (prisma.${key})`).toBeDefined()
            expect(typeof prisma[key].findMany, `prisma.${key}.findMany should be a function`).toBe('function')
        }
    })
})
