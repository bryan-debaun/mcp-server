...
describe('update-project-field tool', () => {
    ...
    it('should register tool with correct name and schema', () => {
        expect(mockServer.registerTool).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                ...
                description:
                    'Update a custom field in a GitHub Project V2 (rename, add/remove options)',
            }),
            expect.any(Function),
        )
    })
    ...
    it('should rename field', async () => {
        vi.mocked(graphql.getProjectFields).mockResolvedValue({
            ...
            fields: [{ id: 'PVTF_field1', name: 'Status', dataType: 'TEXT' }],
        })
        ...
        expect(graphql.updateProjectFieldName).toHaveBeenCalledWith(
            'PVT_test123',
            'PVTF_field1',
            'Review Status',
        )
        expect(graphql.clearProjectCache).toHaveBeenCalledWith(
            'bryan-debaun',
            2,
        )
    ...
    it('should remove options from SINGLE_SELECT field', async () => {
        ...
        expect(graphql.removeFieldOptions).toHaveBeenCalledWith('PVT_test123', [
            'opt2',
        ])
    ...
    it('should fail when trying to add options to non-SINGLE_SELECT field', async () => {
        ...
        expect(result.content[0].text).toContain(
            'Only SINGLE_SELECT fields support options',
        )
    })
