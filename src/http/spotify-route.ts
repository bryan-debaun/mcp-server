...
export function registerSpotifyRoute(app: any) {
    ...
    app.get('/api/spotify/liked', async (req: Request, res: Response) => {
        try {
            const { getLikedTracks } = await import(
    ...
    app.get('/api/spotify/playlists', async (req: Request, res: Response) => {
        try {
            const { getPlaylists } = await import(
                '../adapters/spotify/spotify-adapter.js'
            )
            const limit = Number(req.query.limit ?? 20)
            const offset = Number(req.query.offset ?? 0)
    ...
    ;(async () => {
        ...
        if (!config.spotify.enabled) {
            logger.error(
                'spotify-adapter: auto-start skipped (missing SPOTIFY env)',
            )
            return
        }

        try {
            const { startSpotifyAdapter } = await import(
                '../adapters/spotify/spotify-adapter.js'
            )
            startSpotifyAdapter().catch((err) =>
                logger.error('spotify-adapter: auto-start failed', err),
            )
            logger.error('spotify-adapter: auto-start requested')
        } catch (err) {
            // Non-fatal — adapter may be unavailable in this runtime
            logger.error(
                'spotify-adapter: auto-start import failed',
