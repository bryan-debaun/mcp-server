...
const toolConfig = {
    ...
    description:
        'List all items on a GitHub Projects V2 board, grouped by Status column, with their current field values and issue numbers. Gives a full board snapshot.',
        ...
        async (args: any): Promise<CallToolResult> => {
            try {
                ...
                for (const item of items) {
                    const status =
                        (item.fieldValues['Status'] as string) ?? '(no status)'
